...
__global__ void offlinefps_kernel(int b, int n, int m, int nfilter,
    const int *__restrict__ fmatrix, unsigned char *__restrict__ bitmap, int *__restrict__ output) {
    // fmatrix: (n, nfilter)
    // bitmap: (b, n)
    // output: (b, m)
    ...
    int batch_index = blockIdx.x;
    bitmap += batch_index * n;
    output += batch_index * m;

    int tid = threadIdx.x;
    const int stride = blockDim.x;
    int hop = blockIdx.x * blockDim.x;
    ...
    for (int j = 1; j < m; j++) {
        // update bitmap
        if (threadIdx.x < nfilter) {
            int idx = fmatrix[old*nfilter + threadIdx.x];
            bitmap[idx] = 0;
            bitmap[old] = 0;    // Just in case old is not in the idx set.
        }
        __syncthreads();
        ...
        for (int k = tid; k < n; k += stride) {
            if (old != 0)
                break;
            int kidx = (k+hop) % n;     // we do this to introduce difference to the results within batches.
            if (bitmap[kidx] == 1)
                old = kidx;
        }
        __syncthreads();
...
void offlinefps_kernel_launcher(int b, int n, int m, int nfilter,
    const int *filter_matrix, unsigned char *bitmap, int *output) {

    cudaError_t err;
    unsigned int n_threads = opt_n_threads(n);

    offlinefps_kernel<<<b, n_threads>>>(b, n, m, nfilter, filter_matrix, bitmap, output);
