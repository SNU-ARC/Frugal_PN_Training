...
def voxelize(coord, voxel_size=0.05, hash_type='fnv', mode=0):
    ...
    if mode == 0:  # train mode
        ...
        return idx_unique
    elif mode == 2:  # train with precompute_fps mode
        idx_select = np.cumsum(np.insert(count, 0, 0)[0:-1])  # We don't allow randomness here in precompute_fps mode.
        idx_unique = idx_sort[idx_select]
        return idx_unique
    else:  # val mode
...
# We do not shuffle points in precompute_fps mode.
def crop_pc_precompute_fps(coord, feat, label, split='train',
            voxel_size=0.04, voxel_max=None, downsample=True, fps_idx=None):
    if voxel_size and downsample:
        coord -= coord.min(0) 
        uniq_idx = voxelize(coord, voxel_size, mode=2)  # No randomness in voxelization.
        coord, feat, label = coord[uniq_idx], feat[uniq_idx] if feat is not None else None, label[uniq_idx] if label is not None else None

    if voxel_max is not None and fps_idx is not None:
        N = len(label)  # the number of points
        if N >= voxel_max:
            init_idx = np.random.randint(N)     # Random crop gives significant accuracy improvement.
            crop_idx = np.argsort(
                np.sum(np.square(coord - coord[init_idx]), 1))[:voxel_max]            
            # Fetch valid precomputed fps idx and place them on the front.
            crop_idx = np.concatenate([fps_idx[np.isin(fps_idx, crop_idx)], crop_idx[~np.isin(crop_idx,fps_idx)]])[:voxel_max] 
        else:
            # fill more points for non-variable case (batched data)
            cur_num_points = N
            query_inds = np.arange(cur_num_points)
            ...
            # Fetch valid precomputed fps idx and place them on the front.
            query_inds = np.concatenate([fps_idx[np.isin(fps_idx, query_inds)], query_inds[~np.isin(query_inds, fps_idx)]])[:cur_num_points]
