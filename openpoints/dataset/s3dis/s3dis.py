...
class S3DIS(Dataset):
    ...
    def __init__(self,
        ...
        if self.precompute_fps == 1:
            ...
            fps_path = "data/S3DIS/fps_results_"+stride+"_epoch"+str(epochs)+".dat"
    ...
    def __getitem__(self, idx):
        ...
        if self.presample:
            ...
            coord, feat, label = crop_pc_precompute_fps(
                coord, feat, label, self.split, self.voxel_size, self.voxel_max, downsample=not self.presample, fps_idx=self.fps_list[data_idx][self.epoch])
            ...
            coord, feat, label = cdata[:, :3], cdata[:, 3:6], cdata[:, 6:7]
            coord, feat, label = crop_pc_precompute_fps(
                coord, feat, label, self.split, self.voxel_size, self.voxel_max, downsample=not self.presample)
        else:
            data_path = os.path.join(
                self.raw_root, self.data_list[data_idx] + '.npy')
            cdata = np.load(data_path).astype(np.float32)
