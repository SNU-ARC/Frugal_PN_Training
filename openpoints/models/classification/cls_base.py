...
class BaseCls(nn.Module):
    ...
    def forward(self, data, precompute_fps=False):
        if not precompute_fps:
            global_feat = self.encoder.forward_cls_feat(data)
        else:
            global_feat = self.encoder.forward_cls_feat(data, precompute_fps=precompute_fps)
        return self.prediction(global_feat)

    def get_loss(self, pred, gt, inputs=None):
        return self.criterion(pred, gt.long())

    def get_logits_loss(self, data, gt, precompute_fps=False):
        logits = self.forward(data, precompute_fps=precompute_fps)
