...
# Fused aggregation without positional encoding
class FusedGroupAndReduceOperation(Function):
    ...
    def forward(ctx, features: torch.Tensor, idx: torch.Tensor) -> torch.Tensor:
        ...
        pointnet2_cuda.fused_group_and_reduce_wrapper(B, C, N, nfeatures, nsample, features, idx, output, source_idx)

        ctx.for_backwards = (source_idx, N)
        return output
    ...
    def backward(ctx, grad_out: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        :return:
            grad_features: (B, C, N) gradient of the features
        """
        source_idx, N = ctx.for_backwards

        B, C, npoint = grad_out.size()
        grad_features = torch.zeros([B, C, N], dtype=torch.float, device=grad_out.device, requires_grad=True)

        grad_out_data = grad_out.data.contiguous()
        pointnet2_cuda.fused_group_and_reduce_grad_wrapper(B, C, N, npoint, grad_out_data, source_idx, grad_features.data)
        return grad_features, None


fused_group_and_reduce = FusedGroupAndReduceOperation.apply
...
class QueryAndGroupXYZ(nn.Module):
    ...
    def forward(self, query_xyz: torch.Tensor, support_xyz: torch.Tensor) -> Tuple[
        torch.Tensor]:
        """
        :param query_xyz: (B, npoint, 3) xyz coordinates of the features
        :param support_xyz: (B, N, 3) centroids
        :return:
            new_features: (B, 3, npoint, nsample)
        """

        if self.nsample is None:
            grouped_xyz = support_xyz.transpose(1, 2).unsqueeze(2)
...
class FusedAggregation(nn.Module):
    def __init__(self, radius: float, nsample: int,
                 relative_xyz=True,
                 normalize_dp=False,
                 normalize_by_std=False,
    ...
    def forward(self, query_xyz: torch.Tensor, support_xyz: torch.Tensor, pos_enc: torch.Tensor, nns_idx: torch.Tensor = None, features: torch.Tensor = None) -> Tuple[
        torch.Tensor]:
        """
        :param query_xyz: (B, npoint, 3) xyz coordinates of the features
        :param support_xyz: (B, N, 3) centroids
        ...
        """
        if self.nsample is None:
            grouped_pos_enc = pos_enc
            grouped_features = features.unsqueeze(2) if features is not None else None
            pooled_features = torch.max(grouped_features + grouped_pos_enc, dim=-1, keepdim=False)[0]
            return pooled_features
        else:
            if nns_idx is None:
                nns_idx = ball_query(self.radius, self.nsample, support_xyz, query_xyz)

            grouped_pos_enc = pos_enc # Reuse

            # Fused aggregation
            pooled_features = fused_group_and_reduce_pe(features, nns_idx, grouped_pos_enc)

            # Original version
            #grouped_features = grouping_operation(features, nns_idx) if features is not None else None
            #pooled_features = torch.max(grouped_features + grouped_pos_enc, dim=-1, keepdim=False)[0]
        return pooled_features
...
class QueryAndGroupDA(nn.Module):
    def __init__(self, radius: float, nsample: int,
                 relative_xyz=True,
                 normalize_dp=False,
                 normalize_by_std=False,
    ...
    def forward(self, query_xyz: torch.Tensor, support_xyz: torch.Tensor, features: torch.Tensor = None, query_idx: torch.Tensor = None) -> Tuple[
        torch.Tensor]:
        """
        :param query_xyz: (B, npoint, 3) xyz coordinates of the features
        :param support_xyz: (B, N, 3) centroids
        :param features: (B, C, N) descriptors of the features
        :param query_idx: (B, N, npoint) sampling results
        :return:
        ...
        if query_idx is not None:
            centroid_features = gather_operation(features, query_idx.int())
        else:
            centroid_features = features
        grouped_features = grouped_features - centroid_features.unsqueeze(-1)
        return grouped_features


class FusedQueryAndGroupDA(nn.Module):
    def __init__(self, radius: float, nsample: int,
                 relative_xyz=True,
                 normalize_dp=False,
                 normalize_by_std=False,
    ...
    def forward(self, query_xyz: torch.Tensor, support_xyz: torch.Tensor, features: torch.Tensor = None, query_idx: torch.Tensor = None) -> Tuple[
        ...
        """
        if self.nsample is None:
            output_features = features.unsqueeze(2)
            output_features = torch.max(output_features, dim=-1, keepdim=False)[0]
            return output_features
        else:
            idx = ball_query(self.radius, self.nsample, support_xyz, query_xyz)
            output_features = fused_group_and_reduce(features, idx)

            if query_idx is not None:
                centroid_features = gather_operation(features, query_idx.int())
            else:
                centroid_features = features
            output_features = output_features - centroid_features
        return output_features


class GroupAllDA(nn.Module):
    def __init__(self, ):
        super().__init__()

    def forward(self, new_xyz: torch.Tensor, xyz: torch.Tensor, features: torch.Tensor = None, query_idx: torch.Tensor = None):
        """
        :param xyz: (B, N, 3) xyz coordinates of the features
        :param new_xyz: ignored
        :param features: (B, C, N) descriptors of the features
        :param query_idx: ignored
        :return:
            new_features: (B, C, 1, N)
        """
        grouped_features = features.unsqueeze(2) if features is not None else None
        return grouped_features
...
class DynamicKNNGroup(nn.Module):
    def __init__(self, nsample: int,
                 relative_xyz=True,
                 normalize_dp=False,
                 return_only_idx=False,
        ...
        if dilation == 1:
            ...
            self.knn = DilatedKNN(nsample, dilation=dilation)
        self.dilation = dilation
        self.relative_xyz = relative_xyz
        self.normalize_dp = normalize_dp
        self.return_only_idx = return_only_idx
    ...
    def forward(self, query_xyz: torch.Tensor, support_xyz: torch.Tensor, features: torch.Tensor = None) -> Tuple[
        ...
        if self.dilation == 1:
            _, idx = self.knn(feat_trans, feat_trans)
        else:
            idx = self.knn(feat_trans)
        if self.return_only_idx:
            return idx
        idx = idx.int()
        xyz_trans = support_xyz.transpose(1, 2).contiguous()
...
def create_aggregation(group_args):
    ...
    if method == 'ballquery':
        aggregation = FusedAggregation(radius, nsample, **group_args_copy)
    elif method == 'knn':
        raise NotImplementedError(f'fused aggregation for knn not implemented')
    elif method == 'dynamic_knn':
        raise NotImplementedError(f'fused aggregation for dynamic_knn not implemented')
    return aggregation


def create_xyz_grouper(group_args):
    group_args_copy = copy.deepcopy(group_args)
    method = group_args_copy.pop('NAME', 'ballquery')
    radius = group_args_copy.pop('radius', 0.1)
    nsample = group_args_copy.pop('nsample', 20)
    ...
    logging.info(group_args)
    if method == 'ballquery':
        grouper = QueryAndGroupXYZ(radius, nsample, **group_args_copy)
    elif method == 'knn':
        raise NotImplementedError(f'xyz_grouper for knn not implemented')
    elif method == 'dynamic_knn':
        raise NotImplementedError(f'xyz_grouper for dynamic_knn not implemented')
    return grouper
...
def create_grouper(group_args):
    ...
    if nsample is not None:
        if method == 'ballquery':
            ...
            grouper = DynamicKNNGroup(nsample,  **group_args_copy)
    else:
        grouper = GroupAll()
    return grouper


def create_da_grouper(group_args):
    group_args_copy = copy.deepcopy(group_args)
    method = group_args_copy.pop('NAME', 'ballquery')
    radius = group_args_copy.pop('radius', 0.1)
    nsample = group_args_copy.pop('nsample', 20)
    ...
    logging.info(group_args)
    if nsample is not None:
        grouper = QueryAndGroupDA(radius, nsample, **group_args_copy)
    else:
        grouper = GroupAllDA()
    return grouper


def create_fused_da_grouper(group_args):
    group_args_copy = copy.deepcopy(group_args)
    method = group_args_copy.pop('NAME', 'ballquery')
    radius = group_args_copy.pop('radius', 0.1)
    nsample = group_args_copy.pop('nsample', 20)
    ...
    logging.info(group_args)
    grouper = FusedQueryAndGroupDA(radius, nsample, **group_args_copy)
    return grouper
