...
class OfflineFPS(Function):
    @staticmethod
    def forward(ctx, filter_matrix: torch.Tensor, npoint: int, nbatch: int) -> torch.Tensor:
        """
        Generate "num_of_epochs" number of fps results. This function is called before training.
        :param ctx:
        :param filter_matrix: (N, nfilter) where N > npoint
        :param npoint: int, number of features in the sampled set
        :nbatch: int, number of epochs
        :return:
             output: (nbatch, npoint) tensor containing the set (idx)
        """
        assert filter_matrix.is_contiguous()

        N, nfilter = filter_matrix.size()
        output = torch.cuda.IntTensor(nbatch, npoint)
        bitmap = torch.cuda.ByteTensor(nbatch, N).fill_(1)

        pointnet2_cuda.offlinefps_wrapper(
            nbatch, N, npoint, nfilter, filter_matrix, bitmap, output)
        return output


offline_fps = OfflineFPS.apply
