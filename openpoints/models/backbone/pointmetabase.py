...
from ..layers import create_convblock1d, create_convblock2d, create_act, CHANNEL_MAP, \
    create_grouper, create_xyz_grouper, create_aggregation, furthest_point_sample, random_sample, three_interpolation
import copy
...
class SetAbstraction(nn.Module):
    ...
    def forward(self, pf_pe, precompute_fps=False):
        p, f, pe = pf_pe
        if self.is_head:
            f = self.convs1(f)  # (n, c)
        else:
            if not self.all_aggr:
                if precompute_fps:
                    idx = torch.arange(p.shape[1] // self.stride, device=p.device).repeat(p.shape[0], 1)   # Data are pre-aranged in the "farthest point sampled" order.
                else:
            ...
            if self.fused_agg:
                # Fused Aggregation
                dp, nns_idx = self.xyz_grouper(new_p, p)    # Must return idx! We want to perform ballquery only once...
                pe = self.convs2(dp)
...
class PointMetaBaseEncoder(nn.Module):
    ...
    def forward_cls_feat(self, p0, f0=None, precompute_fps=False):
        if hasattr(p0, 'keys'):
        ...
        for i in range(0, len(self.encoder)):
            pe = None
            if precompute_fps and i == 1: # fps precompute is only done for first layer
                p0, f0, pe = self.encoder[i][0]([p0, f0, pe], True)
                p0, f0, pe = self.encoder[i][1:]([p0, f0, pe])
            else:
                p0, f0, pe = self.encoder[i]([p0, f0, pe])
        return f0.squeeze(-1)

    def forward_seg_feat(self, p0, f0=None, precompute_fps=False):
        if hasattr(p0, 'keys'):
            p0, f0 = p0['pos'], p0.get('x', None)
        if f0 is None:
            f0 = p0.clone().transpose(1, 2).contiguous()
        ...
        for i in range(0, len(self.encoder)):
            if i == 0:
                ...
                if precompute_fps and i == 1:   # fps precompute is only done for first layer.
                    _p, _f, _ = self.encoder[i][0]([p[-1], f[-1], pe], True)    # fps_list only used when training.
                else:
                    _p, _f, _ = self.encoder[i][0]([p[-1], f[-1], pe])    # fps_list only used when training.
                if self.blocks[i] > 1:
                    # grouping
