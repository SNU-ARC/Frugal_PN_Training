...
import logging
from ..layers import furthest_point_sample, random_sample,  LocalAggregation, three_interpolation, create_convblock1d, create_convblock2d, create_aggregation, create_grouper, create_xyz_grouper
from ..build import MODELS


class PointNetSAModuleDA(nn.Module):
    ...
    def __init__(self,
        ...
        # We only support SSG(Single Stage Grouping) in this module.
        radius = radii[0]
        nsample = nsamples[0]
        channels1 = channel_list[0]
        channels2 = [3, channels1[-1]]   # 1 layer explicit positional embedding

        convs1 = []
        for i in range(len(channels1) - 1):  # #layers in each blocks
            convs1.append(create_convblock1d(channels1[i], channels1[i + 1],
                                             norm_args=norm_args,
                                             act_args=None if i == len(channels1) - 2 else act_args,
                                             **conv_args)
                          )
        ...
        self.aggregation = create_aggregation(group_args)
    ...
    def forward(self,
        ...
        if query_xyz is None and self.stride > 1:
            if precompute_fps:
                idx = torch.arange(support_xyz.shape[1] // self.stride, device=support_xyz.device).repeat(support_xyz.shape[0], 1)  # Data are pre-aranged in the "farthest point sampled" order.
            else:
                idx = self.sample_fn(
        ...
        # preconv
        f = self.convs1(support_features)

        if self.fused_agg:
            # Fused Aggregation
            dp, nns_idx = self.xyz_grouper(query_xyz, support_xyz)
            pe = self.convs2(dp)
            f = self.aggregation(query_xyz, support_xyz, pe, nns_idx, f)
        else:
            # grouping
            dp, fj = self.grouper(query_xyz, support_xyz, f)
...
class PointNet2EncoderDA(nn.Module):
    ...
    def forward_cls_feat(self, xyz, features=None, precompute_fps=False):
        if hasattr(xyz, 'keys'):
        ...
        for i in range(len(self.SA_modules)):
            ...
            if precompute_fps and i == 0:
                xyz, features = self.SA_modules[i](xyz, features, None, True)
            else:
                xyz, features = self.SA_modules[i](xyz, features)
        return features.squeeze(-1)

    def forward_seg_feat(self, xyz, features=None, precompute_fps=False):
        if hasattr(xyz, 'keys'):
            xyz, features = xyz['pos'], xyz['x']
        if features is None:
            features = xyz.clone().transpose(1, 2).contiguous()
        ...
        for i in range(len(self.SA_modules)):
            if precompute_fps and i == 0:
                li_xyz, li_features = self.SA_modules[i](l_xyz[i], l_features[i], None, True)
            else:
