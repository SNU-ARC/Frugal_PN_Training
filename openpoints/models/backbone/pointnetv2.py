    ...
    def forward(self,
        ...
        if query_xyz is None and self.stride > 1:
            if fps_idx is not None:
                idx = fps_idx   # We reuse fps results here. Only used when training.
            else:
                idx = self.sample_fn(
...
class PointNet2Encoder(nn.Module):
    ...
    def forward_cls_feat(self, xyz, features=None, fps_list=None):
        if hasattr(xyz, 'keys'):
        ...
        for i in range(len(self.SA_modules)):
            ...
            if fps_list != None:
                if i < len(fps_list):   # For stride 1, no fps value.
                    xyz, features = self.SA_modules[i](xyz, features, None, fps_list[i])
                else:
