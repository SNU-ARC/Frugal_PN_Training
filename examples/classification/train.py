...
def main(gpu, cfg, profile=False):
    ...
    train_loader = build_dataloader_from_cfg(cfg.batch_size,
                                             ...
                                             split='train',
                                             distributed=cfg.distributed,
                                             precompute_fps=int(cfg.model.training_recipe.precompute_fps),
    ...
    for epoch in range(cfg.start_epoch, cfg.epochs + 1):
        ...
        # Run 10 epoch for performance test.
        #if epoch == 11:
        #    break
    ...
    print("Total Training Time (ms): "+str(np.sum(times)*1000))
    #exit(0)    # For performance test
...
def train_one_epoch(model, train_loader, optimizer, scheduler, epoch, cfg):
    ...
    for idx, data in pbar:
        ...
        #with profile(
        #    on_trace_ready=torch.profiler.tensorboard_trace_handler('./tensorboard/train')
        #) as p:
        logits, loss = model.get_logits_loss(data, target, precompute_fps=cfg.model.training_recipe.precompute_fps) if not hasattr(model, 'module') else model.module.get_logits_loss(data, target, precompute_fps=cfg.model.training_recipe.precompute_fps)
        loss.backward()
        #if idx == 2:
        #    exit(0)
